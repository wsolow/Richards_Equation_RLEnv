"""
PPO Continuous Action Implemenation 
...
class PPO_Continuous_Agent(nn.Module):
...
class PPO_Continuous(BaseTrainer):
    ...
    def __init__(self, config):
        ...
        self.agent = PPO_Continuous_Agent(self.envs).to(self.device)

    def train(self):
        """
        Train the agent with continuous action space
        """
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for step in range(0, args.num_steps):
                ...
                next_obs, next_done = torch.tensor(next_obs).to(torch.float32).to(self.device), torch.tensor(next_done).to(self.device)
            ...
            if iteration % args.log_frequency == 0:
                torch.save(self.agent.state_dict(), f"{fpath}/ppo_agent_{iteration}.pt")
